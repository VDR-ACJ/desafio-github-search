...
class MainActivity : AppCompatActivity() {
        ...
        githubApi.getAllRepositoriesByUser(nomeUsuario.text.toString()).enqueue(object : Callback<List<Repository>>{
            override fun onResponse(
                ...
                if (response.isSuccessful){
                    //VERIFICA SE RESPONSE É NULO CASO NÃO SEJA PASSA A LISTA PARA ALIMENTAR O ADAPTER
                    response.body()?.let {
                        setupAdapter(it)
                    }
                }
                else{
                    Toast.makeText(this@MainActivity,getString(R.string.error_response),Toast.LENGTH_LONG).show()
                }
            }
    ...
    fun setupAdapter(list: List<Repository>) {
        ...
        val repositoryAdapter = RepositoryAdapter(list,mainActivity)
